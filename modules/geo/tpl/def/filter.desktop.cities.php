    ...
    letter:
        ...
        ?><ul class="rel"><?
            ...
            while(list($k,$vv) = each($v)) {
                ?><li><a href="<?= $vv['link'] ?>" class="<? if($vv['active']) { ?>active<? } ?>" data="{id:<?= $vv['id'] ?>,pid:<?= $vv['pid'] ?>}" title="<?= $vv['title'] ?>"><span><?= $vv['title'] ?></span><i class="fa fa-chevron-right" aria-hidden="true"></i></a></li><?
