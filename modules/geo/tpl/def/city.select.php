...
$U = mt_rand(1,100);
$placeholder = ( ! empty($options['placeholder']) ? HTML::escape($options['placeholder']) : _t('','Երևան') );
$params = array();
...
<? } else { ?>
    ...
    <input type="text" class="input-large j-geo-city-select-ac" id="j-geo-city-select-ac<?= $U ?>" value="<?= ( ! empty($city['title']) ? HTML::escape($city['title']) : '' ) ?>" placeholder="<?= $placeholder ?>" autocomplete="off" style="width:<?= ! empty($options['width']) ? $options['width'] : '402px;' ?>;<?= ! $country_id ? 'display:none;' : '' ?>"/>
