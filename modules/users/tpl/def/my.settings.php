...
<div class="u-cabinet__settings" id="j-my-settings">
    <? if ($shopID): ?>
        <div class="u-cabinet__settings__block">
            <div class="u-cabinet__settings__block__title ajax ajax-ico j-block-toggler" data-block="shop">
                <span class="user-settings-title"><?= _t('users', 'Настройки магазина') ?></span>
            </div>
    ...
    <? endif; # shop settings ?>
    <? if ($on['contacts']) { ?>
        <span class="user-settings-title"><?= _t('users', 'Контактные данные') ?></span>
        <div class="u-cabinet__settings__block__content j-block j-block-contacts">
            <form class="form-horizontal j-form-contacts" action="">
                <input type="hidden" name="act" value="contacts"/>
                <div class="control-group">
                    <label class="control-label"><?= _t('users', 'Контактное лицо') ?><span
                            class="required-mark">*</span></label>
                    <div class="controls">
                        <input type="text" name="name" value="<?= $name ?>" class="input-block-level j-required"
                               maxlength="50"/>
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label hidden-phone"><?= _t('users', 'Фотография') ?><br/>
                    ...
                    <div class="controls">
                        ...
                        <div class="u-cabinet__settings__photo__download pull-left">
                            <a class="ajax" id="j-my-avatar-upload" href="javascript:void(0);">
                                <?= _t('users', 'загрузить фото') ?>
                            </a>
                        </div>
                    </div>
                </div>
                    ...
                    <div id="j-my-geo-addr">
                        <div class="control-group">
                            ...
                            <div class="controls">
                                ...
                                <input type="text" class="input-block-level" name="addr_addr" id="j-my-geo-addr-addr"
                                       value="<?= $addr_addr ?>"/>
                            </div>
                        </div>
                        <div class="control-group i-formpage__map">
                            <div class="controls">
                                <div id="j-my-geo-addr-map" class="i-formpage__map_desktop"
    ...
    <? } # on['contacts'] ?>
    <? if (!empty($social)) { ?>
        <span class="user-settings-title"><?= _t('users', 'Связь с социальными сетями') ?></span>
        <div class="u-cabinet__settings__block__content j-block j-block-social">
            <form action="">
                <div class="u-sc">
                    <?= _t('users', 'Ինֆորմացիայի փոխանցումն ավելի արագացնելու համար <br />կապեք ձեր սոց․ կայքերը Ogut.am-ին') ?>
                    :
                    <br/><br/>
                    <? foreach ($social as $k => $v) {
                        ...
                        ?>
                        <a href="#"
                           class="btn u-sc_<?= $v['class'] ?><? if (isset($v['user'])) { ?> active<? } ?> j-my-social-btn"
    ...
    <? } ?>
    <? if (!empty($enotify)) { ?>
        <a href="javascript:void(0);"><span class="user-settings-title"><?= _t('users', 'Настройка уведомлений') ?></span></a>
        <div class="u-cabinet__settings__block__content j-block j-block-enotify">
            <form class="form-horizontal j-form-enotify" action="">
                ...
                <div class="u-cabinet__settings__delivery">
                    <? foreach ($enotify as $k => $v) { ?>
                        <label class="checkbox"><input type="checkbox" name="enotify[]"
                                                       value="<?= $k ?>" <? if ($v['a']) { ?> checked="checked"<? } ?>
    ...
    <? } ?>
        <span class="user-settings-title"><?= _t('users', 'Изменить пароль') ?></span>
        <div class="u-cabinet__settings__block__content j-block j-block-pass">
            <form class="form-horizontal j-form-pass" action="">
                <input type="hidden" name="act" value="pass"/>
                <div class="control-group">
                    <label class="control-label"><?= _t('users', 'Текущий пароль') ?><span
                            class="required-mark">*</span></label>
                    <div class="controls">
                        <input type="password" class="input j-required" name="pass0" maxlength="100"
                               autocomplete="off"/>
                    </div>
                </div>
                <div class="control-group">
                ...
                <div class="control-group">
                    <label class="control-label"><?= _t('users', 'Новый пароль еще раз') ?><span
                            class="required-mark">*</span></label>
                    <div class="controls">
    ...
    <? if ($on['phone']) { ?>
        <div class="u-cabinet__settings__block">
            <a href="javascript:void(0);" class="ajax ajax-ico j-block-toggler"
               data-block="phone"><span><?= _t('users', 'Изменить номер телефона') ?></a>
            <div class="u-cabinet__settings__block__content j-block j-block-phone">
                <form class="form-horizontal j-form-phone" action="">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" class="input" name="phone0"
                    ...
                    <div class="control-group">
                        <label class="control-label"><?= _t('users', 'Новый номер') ?><span
                                class="required-mark">*</span></label>
                        ...
                        <div class="controls j-phone-change-step2 hide">
                            <input type="text" name="code" class="input j-phone-change-code"
                                   placeholder="<?= HTML::escape(_t('users', 'Введите код из sms')) ?>"/>
                            <div class="i-control-links">
                                <a href="#"
                                   class="ajax j-phone-change-repeate"><?= _t('users', 'Выслать код повторно') ?></a>
    ...
    <? if ($on['email']) { ?>
        <div class="u-cabinet__settings__block">
            <span class="user-settings-title"><?= _t('users', 'Изменить email-адрес') ?></span>
            <div class="u-cabinet__settings__block__content j-block j-block-email">
                <form class="form-horizontal j-form-email" action="">
                    ...
                    <div class="control-group">
                        <label class="control-label"><?= _t('users', 'Новый email') ?><span
                                class="required-mark">*</span></label>
                        <div class="controls">
                    ...
                    <div class="control-group">
                        <label class="control-label"><?= _t('users', 'Текущий пароль') ?><span
                                class="required-mark">*</span></label>
                        <div class="controls">
                            <input type="password" class="input j-required" name="pass" maxlength="100"
                                   autocomplete="off"/>
                        </div>
                    </div>
                    <div class="control-group">
    ...
    <? if ($on['destroy']) { ?>
        <div class="u-cabinet__settings__block">
            <a href="javascript:void(0);" class="ajax ajax-ico j-block-toggler"
               data-block="destroy"><span><?= _t('users', 'Удалить учетную запись') ?></span> <i
                    class="fa fa-chevron-down"></i></a>
            <div class="u-cabinet__settings__block__content j-block j-block-destroy">
                        ...
                        <div class="control-group">
                            <div class="controls">
                                <input type="submit" class="btn btn-danger"
                                       value="<?= _t('users', 'Удалить учетную запись') ?>"/>
                            </div>
                        </div>
                    </form>
                </div>
