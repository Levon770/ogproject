...
class Users extends UsersBase
{
    ...
    public function login()
    {
            ...
            do {
                ...
                if ($mResult === true) {
                    ...
                    if (static::registerPhone() && !empty($userData['phone_number'])) {
                        ...
                        $this->errors->set(_t('users', 'Տվյալ էլ․ հասցեին ակտիվացված չէ։<br /><a [link_resend]>Ակտիվացնել</a>',                            array('link_resend'=>'href="#" class="ajax j-resend-activation"')));
                    }
                    break;
                }
        ...
        return $this->authPage('auth.login', _t('users', 'Մուտք գործելու համար օգտագործեք ձեր էլ․ հասցեն<br> կամ սոց․ կայքերը '), array(
    ...
    public function register()
    {
        ...
        switch ($this->input->getpost('step', TYPE_STR))
        {
            ...
            case 'finished': # Страница успешной регистрации
            {
                return $this->authPage('auth.message', _t('users', 'Вы успешно зарегистрировались!'), array(
                        'message' => _t('users', 'Այժմ դուք կարող եք վերադառնալ <a [link_home]>գլխավոր էջ</a> կամ <a [link_profile]>անցնել ձեր անձնական էջը</a>.',
                            array(
        ...
        if (Request::isPOST()) {
            ...
            do {
                ...
                if ($this->model->userEmailExists($email)) {
                    $this->errors->set(_t('users', 'Տվյալ էլ․ հասցեն արդեն գրանցված է <a [link_forgot]>Մոռացե՞լ եք ձեր գաղտնաբառը</a>',
                            array('link_forgot' => 'href="' . static::url('forgot') . '"')
                        ), 'email'
                    );
                    break;
        ...
        return $this->authPage('auth.register', _t('users', 'Գրանցվեք էլ․ հասցեի կամ սոց․ կայքերի միջոցով'), array(
                'phone_on'        => $bPhone,
                'captcha_on'      => $bCaptcha,
        ...
        if (!empty($sKey)) {
            ...
            if (Request::isAJAX()) {
                do {
                    ...
                    if (empty($aData)) {
                        $this->errors->set(_t('users', 'Հետևյալ հղումի ժամկետը լրացել է, <a href="[link_fogot]">Ուղարկել նորը</a>.',                                array('link_fogot' => static::url('forgot'))
                            )
                        );
                        break;
                    }
            ...
            return $this->authPage('auth.forgot.start', _t('users', 'Նշեք այն էլ․ հասցեն, որ լրացրել եք գրանցվելիս'), array(                    'social' => $bSocial,
                )
            );
        }
    }
    ...
    public function profile()
    {
        ...
        if ($user['blocked']) {
            return $this->showInlineMessage(_t('users', 'Օգտատերը արգելափակված է հետևյալ պատճառով:<br /><b>[reason]</b>',
            ...
            switch ($sAction) {
                ...
                case 'pass': # смена пароля
                {
                    ...
                    if (!User::isCurrentPassword($pass0)) {
                        $this->errors->set(_t('users', 'Ընթացիք գաղտնաբառը սխալ է'), 'pass0');
                        break;
                    }
                ...
                case 'email': # смена email
                {
                    ...
                    if ($this->model->userEmailExists($email)) {
                        $this->errors->set(_t('users', 'Տվյալ էլ․ հասցեն արդեն գրանցված է'), 'email');
                        break;
                    }
    ...
    public function my_header_menu()
    {
        ...
        # > избранные
        $menu['favs'] = array('i' => '', 't' => _t('users', 'избранные'), 'url' => BBS::url('my.favs'));

        # > счет
        if (bff::servicesEnabled()) {
            $menu['bill'] = array('i' => '', 't' => _t('users', 'счет'), 'url' => Bills::url('my.history'));
        }
        ...
        # > выход
        $menu['logout'] = array('i' => '', 't' => _t('users', 'выход'), 'url' => Users::url('logout'));
            ...
            case 'user-contacts': # просмотр контактов пользователя
            {
                ...
                if (!empty($user['phones'])) {
                    if (!bff::deviceDetector(bff::DEVICE_PHONE)) {
                        ...
                        $response['phones'] = '<span><a href="tel:'. Users::contactAsImage($phones) .'">'. Users::contactAsImage($phones) .'</a>';
                    } else {
                        $phones = '<span>'; $i = 1;
