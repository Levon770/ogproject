...
$drawButton = function($title, array $meta, $active = false, $value = '', $value_plus = false, $visible = true) use ($lng, $filter_seek) {
    ?><a href="#" title="<?= $value ?>"  class="f-catfilter__item custom-btn-filter j-button<? if( ! $visible || ( $filter_seek && ! $meta['seek']) ) { ?> hide<? } ?>" data="{id:<?= $meta['id'] ?>,key:'<?= $meta['key'] ?>',type:'<?= $meta['type'] ?>',parent:<?= $meta['parent'] ?>,seek:<?= ($meta['seek']?'true':'false') ?>}">
    <span class="f-catfilter__item__content">
                <em class="j-value"><?= ( ! empty($value) ? $value : $title ) ?> <i class="fa fa-plus-square extra<? if(!$value_plus){ ?> hide<? } ?>"></i></em>
            </span>
...
$drawDropdown = function($open, $reset = false, $activeCounter = 0, $submit = true, $extra = array()) use ($lng) {
    if( $open ) {
        ?><div class="f-catfilter__popup dropdown-block box-shadow abs hide<?= ! empty($extra['class']) ? ' '.$extra['class'] : '' ?>"<?= ! empty($extra['id']) ? ' id="'.$extra['id'].'"' : '' ?>><?
    } else {
        ...
        <? } if($reset) { ?>
            <span class="f-catfilter__popup__not-important label pull-left">
                <label class="checkbox">
                    ...
                    <input type="checkbox" class="j-reset"<? if( ! $activeCounter){ ?> disabled="disabled" checked="checked"<? } ?> />&nbsp;<?= $lng['btn_reset'] ?>&nbsp;</label>
            </span>
        <? } ?>
        </div><?
    }
...
$drawCheckbox = function($group, $name, $title = '', $selected = 0, $value = 1, $inSeek = true) use ($lng, $filter_seek) {
    ...
    if( $name !== NULL ) {
        ...
        $html[$group] .= '<label class="checkbox j-checkbox"><input type="checkbox" name="'.$name.'" value="'.$value.'" '.($selected ? ' checked="checked"' : '').' />&nbsp;'.$title.'<div class="control__indicator"></div></label>';
        if( ! $group ) $html[$group] .= '</span>';
    } else {
        if( $group ) { ?><div class="f-catfilter__item_check<?= ($inSeek ? ' j-seek' : '') ?>"><? }
                ...
                if( ! empty($d['multi']) ) {
                    ...
                    echo HTML::renderList($d['multi'], $values, function($k,$i,$values) use ($name) {
                        ...
                        return '<li><label class="checkbox"><input type="checkbox" name="'.$name.'[]"
                            '.(in_array($v, $values)?' checked="checked"':'').' value="'.$v.'" data-num="'.$i['num'].'" />'.$i['name'].'
                        ...
                        foreach($d['multi'] as $k=>$m)
                        {
                            ...
                            echo HTML::renderList($dd['multi'], array(), function($k,$i,$values) use ($name_child) {
                                ...
                                return '<li><label class="checkbox"><input type="checkbox" name="'.$name_child.'[]"
                                    '.( ! empty($i['active'])?' checked="checked"':'').' value="'.$v.'" />'.$i['name'].'
                                    <div class="control__indicator"></div></label></li>';
                            }, array(2=>4,3=>15));
                            ?></div><?
                        }
                ...
                if($self->searchRanges)
                {
                    echo HTML::renderList($d['search_ranges'], $value['r'], function($k,$i,$values) use ($name) {
                        return '<li><label class="checkbox"><input type="checkbox" name="'.$name.'[r][]"
                            '.(in_array($k, $values)?' checked="checked"':'').' value="'.$k.'" />'.$i['title'].'</label>
                            <div class="control__indicator"></div></li>';
                    }, array(2=>4,3=>15));
                }
    ...
    if( ! empty($price_sett['ranges']) && is_array($price_sett['ranges']) ) {
        ...
        echo HTML::renderList($price_ranges, array(), function($k,$i,$values) {
            return '<li><label class="checkbox"><input type="checkbox" name="p[r][]"
                '.( ! empty($i['a'])?' checked="checked"':'').' value="'.$k.'" />'.$i['title'].'</label>
    ...
    if (!empty($aDistricts)) {
        ...
        echo HTML::renderList($aDistricts, array(), function($k,$i,$values) {
            return '<li><label class="checkbox"><input type="checkbox" name="rd[]"
                '.( ! empty($i['a'])?' checked="checked"':'').' value="'.$k.'" />'.$i['t'].'
                <div class="control__indicator"></div></label></li>';
        }, array(2=>4,3=>15));
