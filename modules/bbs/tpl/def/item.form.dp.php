...
$drawDynprops = function($self, $numFirst = false) use (&$dynprops, &$aExtraSettings, $ownerColumn, $drawControl, &$children)
{
    ...
    foreach($dynprops as $d)
    {
        ...
        switch($d['type'])
        {
                ...
                if ( ! empty($d['group_one_row']) ) {
                    ...
                    foreach($d['multi'] as $v) {
                        ...
                        $html .= '<label class="radio inline"><input type="radio" name="'.$name.'"
                                '.($v['value'] == $value?' checked="checked"':'').' value="'.$v['value'].'" data-num="'.$v['num'].'" />'.$v['name'].'<div class="control__indicator"></div></label>';
                    }
                } else {
                ...
                if ( ! empty($d['group_one_row']) ) {
                    ...
                    foreach($d['multi'] as $v) {
                        $html .= '<label class="checkbox inline"><input type="checkbox" name="'.$name.'[]"
                                '.(in_array($v['value'],$value)?' checked="checked"':'').' value="'.$v['value'].'" data-num="'.$v['num'].'" />'.$v['name'].'<div class="control__indicator"></div></label>';
                    }
                } else {
                    $html = HTML::renderList($d['multi'], $value, function($k,$i,$values) use ($name) {
                            $v = &$i['value'];
                            return '<li><label class="checkbox"><input type="checkbox" name="'.$name.'[]"
                                '.(in_array($v,$values)?' checked="checked"':'').' value="'.$v.'" data-num="'.$i['num'].'" />'.$i['name'].'<div class="control__indicator"></div></label></li>';
                        },
                        array(2=>4,3=>15),
                        array('class'=>'unstyled span'.(sizeof($d['multi']) > 15 ? 4 : 6))
                    );
            ...
            {
                $value = (isset($d['value'])? $d['value'] : $d['default_value']);
                $html = '<label class="radio inline"><input type="radio" name="'.$name.'" value="2" '.($value == 2?'checked="checked"':'').' />'.$self->langText['yes'].'<div class="control__indicator"></div></label>&nbsp;
                         <label class="radio inline"><input type="radio" name="'.$name.'" value="1" '.($value == 1?'checked="checked"':'').' />'.$self->langText['no'].'<div class="control__indicator"></div></label>';
            } break;
            ...
            {
                $value = (isset($d['value'])? $d['value'] : $d['default_value']);
                $html = '<label class="checkbox"><input type="hidden" name="'.$name.'" value="0" /><input type="checkbox" name="'.$name.'" value="1" '.($value?'checked="checked"':'').' />'.$self->langText['yes'].'
