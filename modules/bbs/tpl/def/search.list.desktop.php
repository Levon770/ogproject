...
$listBanner = function ($listPosition) {
    ...
    if ($html) {
        return '<div href="javascript:void(0);" onclick="document.getElementsByClassName(\'j-type\')[2].click();return false;" class="sr-page__list__item banner">' . $html . '</div> <div class="spacer"></div>';
    }
    return '';
};
...
<? } else if ($list_type == BBS::LIST_TYPE_GALLERY) { ?>
    ...
    <? $i = 1;
    foreach ($items as &$v) {
        if ($i == 1) { ?><div class="thumbnails items-gallery"><? } ?>
...
<? } else if ($list_type == BBS::LIST_TYPE_MAP) {
    ...
    <? foreach ($items as $k => &$v) { ?>
        <div class="sr-page__map__list__item<? if ($v['svc_marked']) { ?> selected<? } ?> rel j-maplist-item"
             data-index="<?= $k ?>">
            <span class="num"><?= $v['num'] ?>.</span>
            <? if ($v['imgs']) { ?><a class="thumb-preview<? if ($v['imgs'] > 1) { ?> thumb-preview_multi<? } ?>"
                                      href="<?= $v['link'] ?>"><span class="photo-count"><i
                        class="fa fa-camera"></i><?= $v['imgs'] ?></span></a><? } ?>
            <? if ($v['fav']) { ?>
                <a href="javascript:void(0);" class="item-fav active j-i-fav" data="{id:<?= $v['id'] ?>}"
                   title="<?= $lng_fav_out ?>"><span class="item-fav__star"><i
                            class="fa fa-star j-i-fav-icon"></i></span></a>
            ...
            <p>
                <small>
                    <?= $v['cat_title'] ?><br/>
                    <i class="fa fa-map-marker"></i> <?= $v['city_title'] ?><?= !empty($v['district_title']) ? ', ' . $v['district_title'] : '' ?>
                    , <?= $v['addr_addr'] ?>
                </small>
            </p>
        </div>
    ...
    <? if (!Request::isAJAX()) { ?>
        ...
        <div class="sr-page__map_ymap span7 j-map">
            <div class="sr-page__map__controls">
                <span id="j-search-map-toggler"><span class="j-search-map-toggler-arrow">&laquo;</span> <a href="#"
                                                                                                           class="ajax j-search-map-toggler-link"><?= _t('search', 'больше карты'); ?></a></span>
            </div>
