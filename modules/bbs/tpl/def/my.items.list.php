...
$lng_from = _t('bbs.my', 'Սկիզբ');
$lng_to = _t('bbs.my', 'Ավարտ');
                                ...
                                <span class="rel inlblk">
                                    <a href="<?= $v['link'].'?from=my' ?>" class="thumb stack rel inlblk">
                                        ...
                                        <img src="<?= $v['img_s'] ?>" alt="<?= $v['title'] ?>" />
                                            ...
                                            <? } ?>
                                        <? } else { ?>
                                        <img src="<?= $img_default ?>" alt="<?= $v['title'] ?>" />
                                            <? if($v['imgs'] > 1) { ?>
                                                <span class="photo-count"><i class="fa fa-camera white"></i> <?= $v['imgs'] ?></span>
                                            <? } ?>
                                        <? } ?>
                        ...
                        <tr>
                            <td colspan="2" class="u-ads__list__item__content__block_nav">
                                <div class="u-ads__list__item__content__block_nav_links">
                                    ...
                                    <? } ?>
                                    <a href="<?= BBS::url('item.edit', array('id'=>$ID,'from'=>'my')) ?>" class="ico"><i class="fa fa-pencil"></i> <span><?= $lng_a_edit ?></span></a>
                ...
                <div class="sr-page__list__item u-ads__list__item__content__block">
                    <table>
                        <tr>
                            ...
                            <td class="sr-page__list__item_descr">
                                <h5>
                                    <a href="<?= $v['link'].'?from=my' ?>"><?= $v['title'] ?></a><br>
                                    <small><?= $v['cat_title'] ?></small><br>
                                    <? if($v['price_on']) { ?><? if($v['price']) { ?><strong class="nowrap"><?= $v['price'] ?></strong><? } ?><? if($v['price_mod']) { ?><br><small><?= $v['price_mod'] ?></small><? } ?><? } ?>
                        ...
                        <tr>
                            <td>
                                <br>
                                <small class="u-ads__list__item__content__block_stat"><?= $lng_from ?>: <?= tpl::dateFormat($v['publicated']) ?> - <?= $lng_to ?>: <?= tpl::dateFormat($v['publicated_to']) ?></small>
                            </td>
                       </tr>
                        <tr>
                            <td class="u-ads__list__item__content__block_nav align-center">
                                    <a href="<?= BBS::url('item.edit', array('id'=>$ID,'from'=>'my')) ?>" class="btn"><i class="fa fa-pencil"></i> <span><?= $lng_a_edit_phone ?></span></a>
                                    ...
                                    <? } ?>
                                    <? if($v['status'] == BBS::STATUS_PUBLICATED && $moderated) { ?>
                                        <a href="#" data-id="<?= $ID ?>" data-act="unpublicate" class="btn red j-i-status"><i class="fa fa-times"></i></a>
                                    <? } else if($v['status'] == BBS::STATUS_PUBLICATED_OUT) { ?>
