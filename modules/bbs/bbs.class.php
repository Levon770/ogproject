...
class BBS extends BBSBase
{
    ...
    {
        $nLimit = config::sys('bbs.index.last.limit', 150);
            ...
            switch ($this->input->getpost('act', TYPE_STR)) {
                ...
                case 'views-stat': # График статистики просмотров ОБ
                {
                    ...
                    $aResponse['lang'] = array(
                        ...
                        'months'         => explode(',', _t('view', 'Հունվարի,Փետրվարի,Մարտի,Ապրիլի,Մայիսի,Հունիսի,Հուլիսի,Օգոստոսի,Սեպտեմբերի,Հոկտեմբերի,Նույեմբերի,Դեկտեմբերի')),
                        'shortMonths'    => explode(',', _t('view', 'հունվար,փետրվար,մարտ,ապրիլ,մայիս,հունիս,հուլիս,օգոստոս,սեպտեմբեր,հոկտեմբեր,նոյեմբեր,դեկտեմբեր')),
        ...
        if (Request::isPOST()) {
            ...
            do {
                ...
                if ($this->spamDuplicatesFound($nUserID, $aData)) {
                    $this->errors->set(_t('bbs', 'Դուք արդեն հրապարակել եք այս հայտարարությունը։'));
                    break;
                }
            ...
            if ($this->errors->no()) {
                switch ($sAction) {
                    ...
                    case 'mass-refresh': # массовое продление
                        ...
                        if (!empty($aResponse['cnt'])) {
                            $aResponse['message'] = _t('bbs.my', 'Նշված հայտարարության ժամկետը երկարացվեծ է');
                        }
                    break;
