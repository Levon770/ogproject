...
//                    if( ! ex.changed ) return;
    ...
    function userPhonesInit(limit, data)
    {
        ...
        function add(value)
        {
            ...
            index++; total++;
            $block.append('<div class="i-formpage__contacts__item">'+'<div class="input-prepend">'+'<span class="add-on"><i class="ico ico__phone-dark"></i></span>'+
                          '<input type="tel" maxlength="30" name="phones['+index+']" value="'+value.replace(/"/g, "&quot;")+'" class="input-large j-phone" '+(o.contactsFromProfile ? ' readonly="readonly"' : '')+' placeholder="'+ o.lang.phones_tip+'" />'+
                                '</div>'+
                                (total==1 && ! o.contactsFromProfile ? '&nbsp;<a class="pseudo-link-ajax j-plus" href="#"><small><i class="fa fa-plus" aria-hidden="true"></i></small></a>':'')+
                           '</div>');
        }
